...
OpenCode（oc）模型调用指导 skill：统一指导调用 OpenCode 的 GO 订阅与 Zen 按量模型，覆盖文本与多模态（图片/音频）场景。

## 功能

- 文本调用：deepseek-v4-flash / pro 等
- 多模态调用：mimo-v2.5、minimax-m3、qwen3.7-plus、Claude、Gemini、GPT-5.x 等
...
- 多模态场景：把图片连同要判断的问题一起发送；音频当前仅 Zen Gemini 部分模型支持

## 凭据配置

从 `~/.codex/config_opencode.toml` 提取 `experimental_bearer_token`，或设置环境变量 `OPENCODE_API_KEY`。本仓库不包含、也不会接受任何真实密钥。
